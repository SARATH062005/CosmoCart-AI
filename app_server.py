...
async def login(request: LoginRequest):
    ...
    if name in profiles:
        user = profiles[name]
        if user['user_info'].get('password') == request.password:
            # Face verification check if signature is present in stream
            data = video_stream.latest_data
            face_matched = False
            if data and data.get('signature'):
                recognized_name, _ = identify_user(data['signature'])
                if recognized_name == name:
                    face_matched = True
            
            return {"status": "success", "user": user, "face_matched": face_matched}
    return {"status": "error", "error": "Invalid name or password"}

@app.post("/signup")
async def signup(request: SignupRequest):
    # Capture face signature during signup
    data = video_stream.latest_data
    if not data or not data.get('signature'):
        return {"status": "error", "error": "Face not detected. Please look at the camera."}
